...
(function ($, window, document) {
    ...
    SearchableOptionList.prototype = {
        ...
        _initializeUiElements: function () {
            ...
            // Sets initial placeholder -- MY CODE
            if (!this.config.multiple) {
                $.each(this.config.data, function(index, option) {
                    if (option.selected) {
                        self.$input.attr('placeholder', this.label)
                    };
                });
            }
            ...
            var self = this,
                ...
                loopFunction = function () {
                    ...
                    while (currentBatch++ < self.config.asyncBatchSize && nextIndex < solItems.length) {
                        ...
                        if (item.type === self.SOL_OPTIONGROUP_FORMAT.type) {
                            self._renderOptiongroup(item);
                        } else if (item.type === self.SOL_OPTION_FORMAT.type) {
        ...
        _renderOption: function (solOption, $optionalTargetContainer, groupLabel) {
            var self = this,
...
/////////////////// MY CODE
                    if(groupLabel) {
                        if($inputElement[0].checked == false) {
                            $("input#"+groupLabel+".ownsol-checkbox")[0].checked = false;
                        } else {
                            var groupBox = $inputElement.parent().parent().parent();
                            if(groupBox.find($('.sol-option input:checkbox:checked')).length == groupBox.find($('.sol-option')).length) {
                                $("input#"+groupLabel+".ownsol-checkbox")[0].checked = true;
                            }
                        }
                    }
                    // Changes placeholder to reflect current selection if single SOL
                    if (!self.config.multiple) {
                        // if sol is single
                        var valselected = self.$selection.find('input:checked').val();
                        if (/^\d+$/.test(valselected)) {
                            valselected = parseInt(valselected);
                        }
                        var dataline = self.config.data.find( line => line.value === valselected);
                        self.$input.attr('placeholder', dataline.label);
                    }
                })
        ...
        _renderOptiongroup: function (solOptiongroup) {
            var self = this,
                $groupSelect = $('<input type="checkbox" class="ownsol-checkbox"/>')
                    .attr('id', solOptiongroup.label)
                    .attr('value', solOptiongroup.label),
                ...
                $groupCaption.append($groupSelect);

            $groupSelect.change(function () {
                $.each(solOptiongroup.children, function (index, item) {
                    if($groupSelect[0].checked == true) {
                        item.displayElement.find('input')[0].checked = true;
                    } else {
                        item.displayElement.find('input')[0].checked = false;
                    }
                });
            });
            ...
            if(i == solOptiongroup.children.length) {
            ...
            if ($.isArray(solOptiongroup.children)) {
                $.each(solOptiongroup.children, function (index, item) {
                    self._renderOption(item, $groupItem, solOptiongroup.label);
                });
            }
